...
//***********************************************************************************************************************************
    ...
    {
        get { return playerReady[PhotonNetwork.player.ID - 1]; }
    }
    ...
    /// </summary>
    void Update () {
        if (phaseCheck.NowPhaseGet == PhaseCheck.Phase.StandbyPhase && playerReady[0] && playerReady[1])
        {
    ...
    {
        photonView.RPC("ReadyCheck", PhotonTargets.AllViaServer, playerCode,isReady);
    }
    /// <summary>
    /// 準備が完了したかの取得を行う
    /// </summary>
    ...
    public void ReadyCheck(int playerCode,bool isReady)
    {
        ...
        if (phaseCheck.NowPhaseGet == PhaseCheck.Phase.StandbyPhase || phaseCheck.NowPhaseGet == PhaseCheck.Phase.CointossPhase)
        {
            //どのプレイヤーが準備完了か否かを取得する。
            playerReady[playerCode] = playerReady[playerCode] != isReady ? isReady : !isReady;
            Debug.Log("Player" + (playerCode + 1) + ":" + playerReady[playerCode]);
        }
    }
}
